...
async function postPurchases(req, res, next) {
  const { userId, purchaseDetails,beerId, totalPrice, status, address, email } = req.body;
  console.log(userId, purchaseDetails,beerId, totalPrice, status, address, email)
  try {
    ...
    purchaseDetails.forEach(async(item) =>{
      let beer = await Beer.findAll({ where: { id: item.beerId }, include: { model: Seller } });
